...
class follower:
    def __init__(self):
        ...
        self.pub = rospy.Publisher("/" + sys.argv[1] + "/cmd_vel", Twist, queue_size = 1)
    ...
    def check_people(self):
        ...
        for i in range(70,-2,-1) + range(359, 289,-1):
            ...
            if   np.nan_to_num( self.msg.ranges[i] ) != 0 :
                 laser_data.append(np.nan_to_num(self.msg.ranges[i]))

            elif (i+1) in range(70,-2,-1) + range(359, 289,-1) and (i-1) in range(70,-2,-1) + range(359, 289,-1) and np.nan_to_num(self.msg.ranges[i]) == 0:
                 laser_data.append((np.nan_to_num(self.msg.ranges[i+1])+np.nan_to_num(self.msg.ranges[i-1]))/2)

            else :
                 laser_data.append(np.nan_to_num(self.msg.ranges[i]))
                 print(self.msg.ranges[i], " ", np.nan_to_num(self.msg.ranges[i]))
